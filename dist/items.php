        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="row mb-4">
              <div class="col-md-12">
                <?php
                // Hide Add Goods and Goods Rate Details buttons in view mode
                $isViewBill = isset($_GET['viewbill']) && $_GET['viewbill'] == '1';
                if (!$isViewBill) {
                ?>
                ...
                <?php
                // Show 'Rate Details Added' if ?rate=added and correct a_c_id, c_id, invoice_id
                $showRateAdded = isset($_GET['rate']) && $_GET['rate'] === 'added';
                $urlParams = [];
                if (isset($_GET['a_c_id'])) $urlParams[] = 'a_c_id=' . urlencode($_GET['a_c_id']);
                if (isset($_GET['c_id'])) $urlParams[] = 'c_id=' . urlencode($_GET['c_id']);
                if (isset($_GET['invoice_id'])) $urlParams[] = 'invoice_id=' . urlencode($_GET['invoice_id']);
                $itemsUrl = 'items.php' . ($urlParams ? '?' . implode('&', $urlParams) : '');
                if ($showRateAdded) {
                  echo '<a href="#" class="btn btn-success ms-2">Rate Details Added</a>';
                } else {
                  echo '<button type="button" class="btn btn-info ms-2" id="openGoodsRateModal">Goods Rate Details</button>';
                }
                }
                ?>
              ...
              <div class="invoice-container">
                ...
                <div class="section-box" style="flex-direction: column;">
                  ...
                  <table style="width: 100%; margin-top: 10px; border-collapse: collapse; font-size: 12px;" border="1">
                    <?php
                    // Fetch invoice details from invoice_details ta    ble
          ...
          ?>
                ...
                ?>
                <p style="margin-bottom:0;"><span style="font-weight: 400; color: #000;">Total Tax Amount (in words):</span> <b>Indian Rupee <?php echo $tax_words; ?> Only</b></p>
                  <div class="footer" style="background: #fff !important; border: none !important; box-shadow: none !important; margin-top:-8px;">
                    <div class="right" style="text-align: right; width: 100%; float: none !important; background: #fff !important;">
                      <?php
                        ...
                        if ($footer_admin_name && $footer_admin_place) {
                          echo '<span style="display:inline-block; margin-bottom:0; margin-top:-1;">for ' . $footer_admin_name . ', ' . $footer_admin_place . '</span><br>';
                        } else {
                          echo '<span style="display:inline-block; margin-bottom:0; margin-top:-1;">for Authorised Firm</span><br>';
                        }
                      ?>
