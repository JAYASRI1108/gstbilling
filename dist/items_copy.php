      ...
      <div class="container-fluid page-body-wrapper">
        ...
        <div class="main-panel">
          <div class="content-wrapper">
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form id="goodsRateForm" class="p-3" style="border:2px solid #ff6600; border-radius:12px; box-shadow:0 0 8px #ff660033;">
                      ...
                      <!-- Common Tax fields below Add Row button -->
                      <div id="taxFieldsSection" style="margin-top: 10px;">
                        <div class="row g-2 mb-2">
                          <div class="col-md-2"><label>CGST %</label>
                            <select class="form-select" name="cgst_percent" id="cgst_percent" required>
                              <option value="select">select</option>
                              <option value="6">6%</option>
                              <option value="9">9%</option>
                            </select>
                          </div>
                          <div class="col-md-2"><label>CGST Amount (₹)</label><input type="number" class="form-control" name="cgst_amount" id="cgst_amount" step="0.01" readonly></div>
                          <div class="col-md-2"><label>SGST %</label>
                            <select class="form-select" name="sgst_percent" id="sgst_percent" required>
                              <option value="select">select</option>
                              <option value="6">6%</option>
                              <option value="9">9%</option>
                            </select>
                          </div>
                          <div class="col-md-2"><label>SGST Amount (₹)</label><input type="number" class="form-control" name="sgst_amount" id="sgst_amount" step="0.01" readonly></div>
                          <div class="col-md-2"><label>Total Amount (₹)</label><input type="number" class="form-control" name="total_amount" id="total_amount" step="0.01" readonly></div>
                          <div class="col-md-2"><label>Taxable Value (₹)</label><input type="number" class="form-control" name="taxable_value" id="taxable_value" step="0.01" readonly></div>
                          <div class="col-md-2"><label>Central Tax Rate (%)</label><input type="number" class="form-control" name="central_tax_rate" id="central_tax_rate" min="0" max="100" step="0.01"></div>
                          <div class="col-md-2"><label>Central Tax Amount (₹)</label><input type="number" class="form-control" name="central_tax_amount" id="central_tax_amount" step="0.01" readonly></div>
                          <div class="col-md-2"><label>State Tax Rate (%)</label><input type="number" class="form-control" name="state_tax_rate" id="state_tax_rate" min="0" max="100" step="0.01"></div>
                          <div class="col-md-2"><label>State Tax Amount (₹)</label><input type="number" class="form-control" name="state_tax_amount" id="state_tax_amount" step="0.01" readonly></div>
                          <div class="col-md-2"><label>Total Tax Amount (₹)</label><input type="number" class="form-control" name="total_tax_amount" id="total_tax_amount" step="0.01" readonly></div>
                        </div>
                      </div>
            ...
            // Render tax fields as form fields below the table
            // Calculate and update common tax fields based on all item rows
            function updateCommonTaxFields() {
              ...
              document.getElementById('taxable_value').value = totalAmount.toFixed(2);
              const cgst_percent = parseFloat(document.getElementById('cgst_percent').value) || 0;
              const sgst_percent = parseFloat(document.getElementById('sgst_percent').value) || 0;
              const central_tax_rate = parseFloat(document.getElementById('central_tax_rate').value) || 0;
              const state_tax_rate = parseFloat(document.getElementById('state_tax_rate').value) || 0;
              let cgst_amount = totalAmount * (cgst_percent / 100);
              let sgst_amount = totalAmount * (sgst_percent / 100);
              document.getElementById('cgst_amount').value = cgst_amount.toFixed(2);
              document.getElementById('sgst_amount').value = sgst_amount.toFixed(2);
              let total_tax_amount = 0;
              let central_tax_amount = totalAmount * (central_tax_rate / 100);
              let state_tax_amount = totalAmount * (state_tax_rate / 100);
              document.getElementById('central_tax_amount').value = central_tax_amount.toFixed(2);
              document.getElementById('state_tax_amount').value = state_tax_amount.toFixed(2);
              total_tax_amount = central_tax_amount + state_tax_amount;
              document.getElementById('total_tax_amount').value = total_tax_amount.toFixed(2);
              let total_invoice_amount = totalAmount + cgst_amount + sgst_amount;
              document.getElementById('total_amount').value = total_invoice_amount.toFixed(2);
            }
            ...
            // Delegate events for common tax fields
            document.getElementById('taxFieldsSection').addEventListener('input', function(e) {
              if ([
                'cgst_percent','sgst_percent','central_tax_rate','state_tax_rate'
              ...
              <div class="invoice-container">
                ...
                <div class="section-box" style="flex-direction: column;">
                  ...
                  <table style="width: 100%; margin-top: 10px; border-collapse: collapse; font-size: 12px;" border="1">
                    ...
                    <tr>
                      <td><strong>Dispatched through:</strong> <?php echo $dispatched_through; ?></td>
                      <td><strong>Destination:</strong> <?php echo $destination; ?></td>
                    </tr>
                    <tr>
                      <td colspan="2"><strong>Terms of Delivery:</strong> <?php echo $terms_of_delivery; ?></td>
                    </tr>
                  </table>
                </div>
                ...
                <div class="table-section">
                  <table>
                    ...
                    if (isset($_GET['invoice_id'])) {
                        ...
                        if (!$conn->connect_error) {
                            $invoice_id = intval($_GET['invoice_id']);
                            $sql = "SELECT grd.*, g.goods_name AS goods_name_actual FROM goods_rate_details grd LEFT JOIN goods g ON grd.g_id = g.g_id WHERE grd.invoice_id = $invoice_id AND grd.status = 'active'";
                            $result = $conn->query($sql);
                            $sl = 1;
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    $desc = htmlspecialchars($row['goods_name_actual'] ?? '');
                                    $hsn = htmlspecialchars($row['hsn_sac']);
                                    $qty = htmlspecialchars($row['quantity']);
                                    $rate = htmlspecialchars($row['rate']);
                                    $per = 'No'; // You can adjust this if you have a unit field
                                    $disc = htmlspecialchars($row['discount_percent']);
                                    $amt = htmlspecialchars($row['amount']);
                                    $cgst_amt = htmlspecialchars($row['cgst_amount']);
                                    $sgst_amt = htmlspecialchars($row['sgst_amount']);
                                    $cgst_per = htmlspecialchars($row['cgst_percent']);
                                    $sgst_per = htmlspecialchars($row['sgst_percent']);
                                    $taxable = htmlspecialchars($row['taxable_value']);
                                    $total_amt = htmlspecialchars($row['total_amount']);
                                    $amount_chargeable += floatval($amt); // Use only the base amount for words
                                    $taxable_value += floatval($taxable);
                                    $total_cgst += floatval($cgst_amt);
                                    $total_sgst += floatval($sgst_amt);
                                    $total_tax += floatval($row['total_tax_amount']);
                                    $grand_total_amount += floatval($total_amt);
                                    $grand_total_qty += floatval($qty);
                                    echo '<tr>';
                                    echo '<td>' . $sl . '</td>';
                                    echo '<td>' . $desc . '<br><br><strong>CGST</strong><br><strong>SGST</strong></td>';
                                    echo '<td>' . $hsn . '</td>';
                                    echo '<td>' . $qty . ' No</td>';
                                    echo '<td>' . $rate . '</td>';
                                    echo '<td>' . $per . '</td>';
                                    echo '<td>' . $disc . '</td>';
                                    echo '<td>' . number_format($amt, 2) . '<br>' . number_format($cgst_amt, 2) . ' (CGST)<br>' . number_format($sgst_amt, 2) . ' (SGST)</td>';
                                    echo '</tr>';
                                    $sl++;
                                }
                            } else {
                ...
                $amt_words = numberToWords(round($grand_total_amount));
                ?>
                <br>
                ...
                <div class="table-section">
                  <table>
                    ...
                    <tbody>
                    <?php
                    // Re-query for tax summary (group by HSN/SAC)
                    if (isset($_GET['invoice_id'])) {
                        $conn = new mysqli('localhost', 'root', '', 'billing');
                        if (!$conn->connect_error) {
                            $invoice_id = intval($_GET['invoice_id']);
                            $sql = "SELECT hsn_sac, SUM(taxable_value) as taxable_value, central_tax_rate, SUM(central_tax_amount) as central_tax_amount, state_tax_rate, SUM(state_tax_amount) as state_tax_amount, SUM(total_tax_amount) as total_tax_amount FROM goods_rate_details WHERE invoice_id = $invoice_id AND status = 'active' GROUP BY hsn_sac, central_tax_rate, state_tax_rate";
                            $result = $conn->query($sql);
                            if ($result && $result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    echo '<tr>';
                                    echo '<td>' . htmlspecialchars($row['hsn_sac']) . '</td>';
                                    echo '<td>' . number_format($row['taxable_value'], 2) . '</td>';
                                    echo '<td>' . htmlspecialchars($row['central_tax_rate']) . '%</td>';
                                    echo '<td>' . number_format($row['central_tax_amount'], 2) . '</td>';
                                    echo '<td>' . htmlspecialchars($row['state_tax_rate']) . '%</td>';
                                    echo '<td>' . number_format($row['state_tax_amount'], 2) . '</td>';
                                    echo '<td>' . number_format($row['total_tax_amount'], 2) . '</td>';
                                    echo '</tr>';
                                }
                            } else {
                                echo '<tr><td colspan="7" class="center">No tax details found.</td></tr>';
                            }
                            $conn->close();
                        }
                    }
                ...
                <?php
                // $total_tax is already the sum of total_tax_amount from all items
                $tax_words = numberToWords(round($total_tax));
                ?>
                <br>
<p><span style="font-weight: 400; color: #000;">Tax Amount (in words):</span> <b>Indian Rupee <?php echo $tax_words; ?> Only</b></p>
                  <div class="footer">
                    <div class="left">
                    ...
                    <div class="right">
                      <?php
                        // Display logged-in admin's name and place in the footer
                ...
                <p class="footer-note"><strong>This is a Computer Generated Invoice</strong></p>
              </div>
            </div>
            <style>
